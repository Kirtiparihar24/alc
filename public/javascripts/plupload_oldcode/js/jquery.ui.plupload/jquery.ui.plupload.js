...
(function(window, document, plupload, $, undef) {
    ...
    function renderUI(obj) {
        obj.html(
            ...
            '<table class="plupload_filelist tabular_listing">' +
            ...
            '<a class="plupload_button plupload_reset">'+_('Reset Upload') + '</a>&nbsp;' +
    ...
    $.widget("ui.plupload", {
        ...
        _create: function() {
            ...
            this.reset_button = $('.plupload_reset', this.container).attr('id', id + '_reset');
            if ($.ui.button) {
            ...
            uploader.bind('Init', function(up, res) {
                ...
                self.reset_button.click(function(e) {
                    uploader.splice();
                    e.preventDefault();
                });
            });
                                ...
                                details = plupload.translate('Resoultion out of boundaries! <b>%s</b> runtime supports images only up to %wx%hpx.').replace(/%([swh])/g, function($0, $1) {
                                    switch ($1) {
                                        case 's':
                                            return up.runtime;
                                        case 'w':
                                            return up.features.maxWidth;
                                        case 'h':
                                            return up.features.maxHeight;
                                    }
                                });
                                break;
        ...
        _handleState: function() {
            ...
            if (uploader.state === plupload.STARTED) {
                ...
                $(self.reset_button).button('disable');
                $([])
        ...
        _updateFileList: function() {
            ...
            $.each(uploader.files, function(i, file) {
                ...
                //var category = jQuery("#upload_multi").attr('category');
                filelist.append(
                ...
                filelist.append(customui(file,filelist,fields));
                self._handleFileStatus(file);
            ...
            if (uploader.files.length === (uploader.total.uploaded + uploader.total.failed)) {
                ...
                self.start_button.button('enable');
                self.reset_button.button('enable');
            }
    ...
    function customui(file,filelist,fields){
        var afile= file;
        var afilelist=filelist;
        var afields=fields;
        if (this.controller=="repositories"){
            afilelist.append(
                '<tr class="ui-state-default plupload_file" id="' + afile.id + '">' +
                '<td><div class="fl"><label for="tag">Tag : </label><input id="' +afile.id +
                '_tag" type="text" class ="plupload_cell"/> &nbsp; &nbsp; </div>' +
                '<div class="fl"><label for="category">Category : </label>'+
                '<select id="' +afile.id +
                '_category" name="category">'+ addOption() +'</select></div>'+                
                '<div class="fr"><label for="description">Description : </label><input id="' +afile.id +
                '_description" type="text" class ="plupload_cell"/>' +
                "</div></td><td colspan='3'></td>"+
                "</tr>"
                );
        }
        if (this.controller=="document_homes"){
            afilelist.append(
                '<tr class="ui-state-default plupload_file" id="' + afile.id + '">' +
                '<td><label for="tag">Tag : </label><input id="' +afile.id +
                '_tag_array" type="text" class ="plupload_cell"/>' +
                '<div class="fr"><label for="description">Description : </label><input id="' +afile.id +
                '_description" type="text" class ="plupload_cell"/></div>'+
                "</td><td colspan='3'></td>"+
                "</tr>"
                );
...
function addOption() {
    ...
    if((this.controller)=="repositories")
    {
        var category = JSON.parse(jQuery("#upload_multi").attr('category'));
        $.each(category, function(index,val) {
            optionString += "<option value="+ val[1]+">" +val[0] +"</option>"
        });
    }
    return optionString;
}
