...
(function () {
    ...
    var g = {
        ...
        formatSize: function (n) {
            ...
            if (n > 1073741824) {
                return (n / 1073741824).toPrecision(3) + " GB"
            }
            if (n > 1048576) {
                return (n / 1048576).toPrecision(3) + " MB"
            }
            if (n > 1024) {
                return (n / 1024).toPrecision(3) + " KB"
            }
            return n + " Bytes"
        },
        getPos: function (o, s) {
                    ...
                    for (D = 0; D < E.length; D++) {
                        ...
                        if (C.size !== b && C.size > q.max_file_size) {
                            B.trigger("Error", {
                                ...
                                message: g.translate("File size error. Min-size 1 Byte and Max-size 100 MB"),
                                file: C
                            });
                            continue
                        }
                        if (C.size !== b && (C.size==0 || C.size > q.max_file_size)) {
                            B.trigger('Error', {
                                code : g.FILE_SIZE_ERROR,
                                message : g.translate('File size error. Min-size 1 Byte and Max-size 100 MB'),
                                file : C
                        ...
                        // Duplicate
                        if (q.prevent_duplicates && curFileNames[C.name] === 1) {
                            continue;
                        }
                        s.push(C);
                        G++
